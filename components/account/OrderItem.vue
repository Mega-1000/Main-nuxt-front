...
<template>
  <div
    class="relative space-y-3 rounded-xl shadow-lg p-3 w-[60vw] max-w-7xl mx-auto border border-white bg-white"
  >
    <p class="text-md">
    ...
    <div class="relative overflow-x-auto">
      <table
        class="max-w-4xl text-sm text-left text-gray-500 w-full"
        v-if="item.packages && item.packages.length > 0"
      >
    ...
    <div class="grid md:flex">
      <template v-if="item?.order_offers && item.order_offers.length > 0">
        <a
          is="button"
          :href="`${config.baseUrl}/order-proform-pdf/${item.order_offers[0]?.id}`"
        ...
        <a
          is="button"
          target="_blank"
          :href="`${config.baseUrl}/order-offer-pdf/${item.order_offers[0]?.id}`"
      ...
      <a
        ...
        class="text-center p-1 bg-slate-100 text-xs text-gray-900 border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700"
      >
        Dane do dostawy i faktury oraz ich edycja
      </a>
      <a
        v-for="invoice in item?.invoices?.filter((invoice: any) => invoice.is_visible_for_client)"
        target="_blank"
        is="button"
        :href="`${config.baseUrl}/storage/invoices/${invoice.invoice_name}`"
        class="p-1 bg-slate-100 text-xs text-gray-900 border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700"
      >
        Pobierz fakturę {{ invoice.invoice_name }}
      </a>
      <button
        v-for="invoice in item?.user_invoices"
    ...
    <div
      class="grid md:flex pt-2"
      v-for="buttonGroup in Object.keys(item.buttons)"
    >
      <p class="text-sm text-center pr-2 pt-1">{{ buttonGroup }}</p>
      <a
        v-for="button in (Object.values(item.buttons[buttonGroup]) as any)"
        target="_blank"
        is="button"
        :href="button.url"
        class="p-1 text-center bg-slate-100 text-xs text-gray-900 border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700"
